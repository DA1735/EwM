...
ついで、確率微分方程式とは何かについて、
歴史的ないきさつを交えて述べる。
確率積分に対する Bachelier、Lévy らの直感的なアイデア、
...
\begin{enumerate}[{[1]}]
  ...
  \item[{[1]}] 舟木直久，確率微分方程式，岩波講座現代数学の基礎，岩波書店，1997．
  \item[{[2]}] 長井英生，確率微分方程式，共立出版，1999．
  \item[{[3]}] 渡辺信三，確率微分方程式，産業図書，1975．
\end{enumerate}
...
%======================================================================
\begin{center}
{\Large\bfseries マリアバン解析について---Brown 運動の汎関数を解析する}
...
無限次元ではあっても Fréchet 微分というものがすでに定義されている。
しかしこの微分概念は確率微分方程式の解のようなものを考えると
有効なものではないことが分かる。
実際確率微分方程式の解は、Fréchet の意味で微分できないばかりでなく、
連続ですらない。
...
\begin{enumerate}[{[1]}]
  \setlength{\itemsep}{2pt}
  \item[{[1]}] 楠岡成雄，Malliavin calculus とその応用，
        \textit{数学} \textbf{36} (1984), 97--109.
  \item[{[2]}] 楠岡成雄，無限次元解析としての確率解析，
        \textit{数学} \textbf{45} (1993), 289--298.
  \item[{[3]}] D.~Nualart,
        \textit{The Malliavin Calculus and Related Topics},
        Probability and its Applications, Springer-Verlag, New York, 1995.
  \item[{[4]}] 重川一郎，確率解析，岩波講座現代数学の展開，岩波書店，東京，1998．
  \item[{[5]}] S.~Watanabe,
        \textit{Lectures on Stochastic Differential Equations and Malliavin Calculus},
        Tata Institute of Fundamental Research, Springer-Verlag, Berlin--Heidelberg--New York, 1984.
  \item[{[6]}] 渡辺信三，確率解析とその応用，
        \textit{数学} \textbf{45} (1990), 97--110.
\end{enumerate}
...
%======================================================================
\begin{center}
{\Large\bfseries 確率解析と幾何---ユビキタス・ウィナー・インテグラル}
...
この講演では確率解析が幾何学に絡んでくるいくつかの局面を紹介いたします。
...
い問題が生起してきます。
このような方向のウィナー積分研究は、M.~Kac が1947年にコーネル
大学物理学講演会で R.~Feynman の博士論文に関する発表を聞いたと
...
\item 曲率とブラウン運動
\item 確率解析による熱核の漸近展開
\item Gauss--Bonnet--Chern の定理の確率解析による証明
\end{enumerate}
について紹介します。
時間があれば、幾何を離れますが、KdV 方程式の $n$ ソリトン解がウィ
...
\begin{enumerate}[{[1]}]
  \setlength{\itemsep}{2pt}
  \item[{[1]}] K.~D.~Elworthy,
        \textit{Stochastic Differential Equations on Manifolds},
        Cambridge Univ.\ Press, 1982.
  \item[{[2]}] K.~D.~Elworthy,
        Geometric aspects of diffusions on manifolds,
        in \textit{École d'Été Probabilités de Saint-Flour XV--XVI 1987},
        Lect.\ Notes in Math.\ \textbf{1362}, Springer, 1989, 276--425.
  \item[{[3]}] N.~Ikeda and S.~Watanabe,
        \textit{Stochastic Differential Equations and Diffusion Processes},
        2nd ed., North-Holland/Kodansha, Amsterdam/Tokyo, 1989.
  \item[{[4]}] D.~W.~Stroock,
        \textit{An Introduction to the Analysis of Paths on a Riemannian Manifold},
        AMS, 2000.
\end{enumerate}
